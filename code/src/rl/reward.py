...
class RewardFunction:
    ...
    def compute_incremental_reward(self,
        ...
        try:
            # 1. 计算主奖励
            if self.scenario_aware_enabled and self.relative_reward_calculator and scenario_context:
                # 使用相对改进奖励
                main_reward = self.relative_reward_calculator.compute_reward(
                    self.previous_quality_score, 
                    current_quality_score, 
                    scenario_context, 
                    episode_num=getattr(self, 'current_episode', 0)
                )
            else:
                # 使用原始势函数奖励
                gamma = 0.99  # 折扣因子
