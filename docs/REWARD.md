...
本系统实现了基于势函数理论的双层激励结构，用于电力网络分区的强化学习训练。该系统由**即时奖励（reward）**和**终局奖励（Reward）**组成，旨在提供密集反馈的同时确保智能体追求高质量的最终分区方案。
...
### 预设配置
系统提供`dual_layer_training`预设配置，针对双层奖励优化了训练参数。
